<template>
  <div class="relative flex flex-col items-start">
    <button
      class="add-image-btn flex items-center gap-1 mb-2"
      type="button"
      @click="triggerFileInput"
      v-if="!imageUrl"
    >
      <UploadIcon :size="18" /> Adicionar imagem
    </button>
    <input
      ref="fileInput"
      type="file"
      ...
      @change="onFileChange"
    />
    <div v-if="imageUrl" class="relative w-full" :style="containerStyle">
      <img
        :src="imageUrl"
        :class="[expanded ? 'object-contain' : 'object-cover', 'rounded-md w-full']"
        :style="imgStyle"
        @click="toggleExpand"
...
<style scoped>
.add-image-btn {
  @apply px-3 py-1 rounded-md border border-input bg-background text-sm hover:bg-accent hover:text-accent-foreground transition;
}
.icon-btn {
  @apply bg-white/80 hover:bg-accent text-muted-foreground rounded-full p-1 shadow border border-input;
}
img {
